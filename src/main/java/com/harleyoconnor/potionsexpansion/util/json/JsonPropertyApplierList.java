...
public final class JsonPropertyApplierList<T> {
    ...
    public <V> JsonPropertyApplierList<T> register(final String key, final Class<V> valueClass, final VoidPropertyApplier<T, V> applier) {
        return this.register(new JsonPropertyApplier<>(key, this.objectType, valueClass, applier));
    }

    public <V> JsonPropertyApplierList<T> register(final String key, final Class<V> valueClass, final PropertyApplier<T, V> applier) {
        return this.register(new JsonPropertyApplier<>(key, this.objectType, valueClass, applier));
    }
